...
module Embulk
  module Input
    class Mixpanel < InputPlugin
      ...
      def self.validate_config(raw_values = {})
        if raw_values.has_key?(:from_date)
          from_date_str = raw_values[:from_date]
          begin
        ...
        if raw_values.has_key?(:fetch_days)
          days = raw_values[:fetch_days]
          if days && days < 1
            # `days` only allowed nil or positive number
            raise ConfigError, "fetch_days '#{days}' is invalid. Please specify bigger number than 0."
          end
        end

        if raw_values.has_key?(:timezone)
      ...
      def self.generate_target_dates(from_date_str, days)
